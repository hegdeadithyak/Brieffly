...
async function run(chapterName:string) {
    try {
        ...
        const result = await model.generateContent(
            `Generate some Test cards on topic ${chapterName} All Theroretical and  Mathematical formulas and Descriptive and write four formulas as options as A,B,C,D, part in latex in JSON format so it is useful to revise concepts. Include "level", "title", "Options","answer" Only return json nothing else`
        );
